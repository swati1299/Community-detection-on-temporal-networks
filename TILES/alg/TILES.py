...
class TILES(object):
    ...
    def __init__(self, filename=None, g=None, ttl=float('inf'), obs=7, path="", start=None, end=None):
        ...
        self.spl = StringIO()
        self.base = os.getcwd()
        if not os.path.exists("%s/%s"%(self.base,self.path)):
            os.makedirs("%s/%s"%(self.base,self.path))
        self.status = open("%s/%s/extraction_status.txt" % (self.base, self.path), "w")
        self.removed = 0
        self.added = 0
    ...
    def execute(self):
        ...
        print(first_line)
        l=list(map(int,first_line.split('\t')))
        print(l)
        actual_time = datetime.datetime.fromtimestamp(float(l[2]))
